...
import AIRAAModal from './AIRAAModal';
...
const AnimatedGrid: React.FC<AnimatedGridProps> = ({ children }) => {
    ...
    return (
        <div className="relative w-full h-56 rounded-xl overflow-hidden p-4 flex flex-col justify-between" 
            ...
            {/* MODIFIED: center children and add vertical breathing room */}
            <div className="relative z-10 flex flex-col justify-center items-center h-full gap-3 pt-2 pb-4">
                {children}
            </div>
        </div>
    );
...
const AIRAACard: React.FC = () => {
    ...
    return (
        ...
        <div className="w-80 rounded-2xl p-4 text-white border border-white/10 glass-panel glass-panel--button cursor-pointer" style={hoverStyle} onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)} onClick={openModal}>
            <AnimatedGrid>
                ...
                <div className="flex justify-center items-center gap-2">
                    <span className="text-sm font-medium glass-panel glass-panel--chat-element px-4 py-1.5 rounded-full" style={{ color: theme.text }}>AI Fashion</span>
                    <span className="text-sm font-medium glass-panel glass-panel--chat-element px-4 py-1.5 rounded-full" style={{ color: theme.text }}>Recommendations</span>
                </div>
            </AnimatedGrid>
        ...
        {/* AIRAA Modal */}
        <AIRAAModal isOpen={isModalOpen} onClose={closeModal} />
        </>
    );
};

export default AIRAACard;
