...
const theme: ThemeColors = { main: '#a855f7', glow: '#a855f7', text: '#d8b4fe', pillBg: 'rgba(168, 85, 247, 0.1)', pillText: '#c084fc', gradientFrom: '#c084fc', gradientTo: '#a855f7', gridCell: 'rgba(216, 180, 254, 0.05)', bgColor: '#16091f' };
const title = "Dressify";
...
// Icons and Pills remain the same as they are generic
const DressIcon = () => (
    <svg width="40" height="40" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="text-white">
        <path d="M9 3H15L17 8H7L9 3Z" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
        <path d="M17 8L20 12V21H4V12L7 8" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
        <path d="M7 8H17" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
        <path d="M12 8V21" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
    </svg>
);
...
const TechPill: React.FC<TechPillProps> = ({ icon, label }) => (<div className="flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-medium" style={{ backgroundColor: theme.pillBg, color: theme.pillText }}>{icon}<span>{label}</span></div>);


const DressifyCard: React.FC = () => {
    ...
    return (
        <div className="w-80 bg-[#1c1c1c] rounded-2xl p-4 text-white border border-white/10" style={hoverStyle} onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
            <AnimatedGrid>
                <div /> 
                ...
                <div className="flex justify-center items-center gap-2">
                    <span className="text-sm font-medium bg-black/30 backdrop-blur-sm px-4 py-1.5 rounded-full" style={{ color: theme.text }}>AI Fashion</span>
                    <span className="text-sm font-medium bg-black/30 backdrop-blur-sm px-4 py-1.5 rounded-full" style={{ color: theme.text }}>Recommendations</span>
                </div>
            </AnimatedGrid>

            <div className="pt-5 pb-2">
                <h2 className="text-2xl font-bold text-gray-50">{title}</h2>
                <p className="text-gray-400 mt-1.5 text-base leading-relaxed">A fashion-forward outfit recommendation platform leveraging AI to personalize your style.</p>
            </div>
